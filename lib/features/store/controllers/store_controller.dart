...
import 'package:ppdelistore/common/models/vat_tax_model.dart';
...
import 'package:ppdelistore/features/ai/domain/models/attribute_data_model.dart';
import 'package:ppdelistore/features/ai/domain/models/other_data_model.dart';
import 'package:ppdelistore/features/ai/domain/models/variation_data_model.dart';
import 'package:ppdelistore/features/category/controllers/category_controller.dart';
import 'package:ppdelistore/features/category/domain/models/category_model.dart';
...
import 'package:ppdelistore/features/store/domain/models/attr.dart';
import 'package:ppdelistore/features/store/domain/models/band_model.dart';
import 'package:ppdelistore/features/store/domain/models/suitable_tag_model.dart';
...
import 'package:ppdelistore/features/store/domain/models/pending_item_model.dart';
...
import 'package:ppdelistore/features/store/domain/models/unit_model.dart';
import 'package:ppdelistore/features/rental_module/profile/controllers/taxi_profile_controller.dart';
import 'package:ppdelistore/helper/image_size_checker.dart';
...
import 'package:ppdelistore/features/store/domain/services/store_service_interface.dart';
...
class StoreController extends GetxController implements GetxService {
  ...
  Future<void> getAttributeList(
    Item? item, {
    bool resetThumbnail = true,
    bool resetMedia = true,
  }) async {
    _attributeList = null;
  ...
  Future<void> addItem(Item item, bool isAdd, {String? genericNameData}) async {
    if (!_isItemThumbnailValid ||
        !_isMetaImageValid ||
        _rawImageValidList.contains(false)) {
      _showImageSizeError();
      return;
    }
    _isLoading = true;
  ...
  void pickImages() async {
    if ((_savedImages.length + _rawImages.length) >=
        (Get.find<SplashController>()
            .configModel!
            .validationConfig!
            .maxUploadFileCount)) {
      showCustomSnackBar(
        'maximum_image_limit_is'.trParams({
          'count': Get.find<SplashController>()
              .configModel!
              .validationConfig!
              .maxUploadFileCount
              .toString(),
  ...
  Future<void> generateAndSetOtherData({
    ...
    await aiController.generateOtherData(
      title: title,
    ...
    if (otherData != null) {
      ...
      if (otherData.generalData?.data?.addonsIds != null &&
        ...
        for (
          ...
          setSelectedAddonIndex(
            addonsIds.indexOf(otherData.generalData!.data!.addonsIds![index]),
            false,
          );
        }
        update();
      ...
      maxOrderQuantityController?.text =
          (priceData?.maxOrderQuantity ?? priceData?.minimumOrderQuantity ?? 0)
              .toString();
  ...
  Future<void> generateAndSetVariationData({
    required String title,
    required String description,
  }) async {
    AiController aiController = Get.find<AiController>();

    await aiController.generateVariationData(
      title: title,
      description: description,
    );
  ...
  Future<void> generateAndSetAttributeData({
    required String title,
    required String description,
  }) async {
    AiController aiController = Get.find<AiController>();

    await aiController.generateAttributeData(
      title: title,
      description: description,
    );
  ...
  Future<bool> generateAndSetDataFromImage({
    ...
    if (response.statusCode == 200) {
      ...
      await aiController.generateTitleAndDes(
        title: title,
