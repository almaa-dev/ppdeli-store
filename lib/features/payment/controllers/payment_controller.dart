...
import 'package:ppdelistore/features/payment/domain/models/bank_info_body_model.dart';
import 'package:ppdelistore/common/models/response_model.dart';
import 'package:ppdelistore/features/payment/domain/models/wallet_payment_model.dart';
import 'package:ppdelistore/features/payment/domain/models/widthdrow_method_model.dart';
import 'package:ppdelistore/features/payment/domain/models/withdraw_model.dart';
...
import 'package:ppdelistore/features/payment/domain/services/payment_service_interface.dart';
...
class PaymentController extends GetxController implements GetxService {
  ...
  Future<void> updateBankInfo(BankInfoBodyModel bankInfoBody) async {
    _isLoading = true;
    update();
    bool isSuccess = await paymentServiceInterface.updateBankInfo(
      bankInfoBody.toJson(),
    );
    if (isSuccess) {
  ...
  Future<void> getWithdrawList() async {
    List<WithdrawModel>? withdrawList = await paymentServiceInterface
        .getWithdrawList();
  ...
  Future<List<WidthDrawMethodModel>?> getWithdrawMethodList() async {
    List<WidthDrawMethodModel>? widthDrawMethods = await paymentServiceInterface
  ...
  Future<void> getWalletPaymentList() async {
    _transactions = null;
    List<Transactions>? transactions = await paymentServiceInterface
