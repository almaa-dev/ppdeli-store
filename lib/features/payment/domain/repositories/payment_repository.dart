...
import 'package:ppdelistore/features/payment/domain/repositories/payment_repository_interface.dart';
...
class PaymentRepository implements PaymentRepositoryInterface {
  ...
  Future<bool> update(Map<String, dynamic> body) async {
    Response response = await apiClient.putData(
      AppConstants.updateBankInfoUri,
      body,
    );
  ...
  Future<bool> requestWithdraw(Map<String?, String> data) async {
    Response response = await apiClient.postData(
      AppConstants.withdrawRequestUri,
      data,
    );
    return (response.statusCode == 200);
  }
  ...
  Future<List<WidthDrawMethodModel>?> getWithdrawMethodList() async {
    ...
    Response response = await apiClient.getData(
      AppConstants.withdrawRequestMethodUri,
    );
    if (response.statusCode == 200) {
      widthDrawMethods = [];
  ...
  Future<List<Transactions>?> getWalletPaymentList() async {
    ...
    Response response = await apiClient.getData(
      AppConstants.walletPaymentListUri,
    );
    if (response.statusCode == 200) {
      transactions = [];
