...
import 'package:ppdelistore/features/deliveryman/domain/repositories/deliveryman_repository_interface.dart';
...
class DeliverymanRepository implements DeliverymanRepositoryInterface {
  ...
  Future<bool> delete(int? id) async {
    Response response = await apiClient.postData(AppConstants.deleteDmUri, {
      '_method': 'delete',
      'delivery_man_id': id,
    });
    return (response.statusCode == 200);
  ...
  Future<List<ReviewModel>?> get(int? id) async {
    ...
    Response response = await apiClient.getData(
      '${AppConstants.dmReviewUri}?delivery_man_id=$id',
    );
    if (response.statusCode == 200) {
      dmReviewList = [];
