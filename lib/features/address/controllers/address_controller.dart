...
import 'package:ppdelistore/features/address/domain/models/address_model.dart';
import 'package:ppdelistore/features/auth/domain/models/module_model.dart';
import 'package:ppdelistore/features/address/domain/models/prediction_model.dart';
import 'package:ppdelistore/features/address/domain/models/zone_model.dart';
import 'package:ppdelistore/features/address/domain/models/zone_response_model.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:ppdelistore/features/address/domain/services/address_service_interface.dart';
...
class AddressController extends GetxController implements GetxService {
  ...
  Future<void> getModules(int? zoneId) async {
    List<ModuleModel>? moduleList = await addressServiceInterface.getModules(
      zoneId,
    );
    if (moduleList != null) {
      _moduleList = [];
  ...
  void setLocation(
    ...
    if (zoneId != null) {
      _inZone = await addressServiceInterface.checkInZone(
        location.latitude.toString(),
        location.longitude.toString(),
        zoneId,
      );
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    String address = await addressServiceInterface.getAddressFromGeocode(
      latLng,
    );
    return address;
  }
  ...
  Future<List<PredictionModel>> searchLocation(
    ...
    if (text.isNotEmpty) {
      List<PredictionModel> predictionList = await addressServiceInterface
          .searchLocation(text);
  ...
  void preloadPickupZones({required List<String> pickupZoneList}) {
    ...
    for (String id in pickupZoneList) {
      final ZoneModel? zone = zoneList?.firstWhereOrNull(
        (zone) => zone.id == int.parse(id),
      );
      if (zone != null) {
