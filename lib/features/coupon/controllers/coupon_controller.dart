...
import 'package:ppdelistore/features/coupon/domain/services/coupon_service_interface.dart';
...
class CouponController extends GetxController implements GetxService {
  ...
  Future<void> getCouponList() async {
    List<CouponBodyModel>? coupons = await couponServiceInterface.getCouponList(
      1,
    );
    if (coupons != null) {
      _coupons = [];
  ...
  Future<CouponBodyModel?> getCouponDetails(int id) async {
    _couponDetails = null;
    CouponBodyModel? couponDetails = await couponServiceInterface
        .getCouponDetails(id);
  ...
  Future<bool> changeStatus(int? couponId, bool status) async {
    bool success = await couponServiceInterface.changeStatus(
      couponId,
      status ? 1 : 0,
    );
    return success;
  ...
  Future<bool> deleteCoupon(int? couponId) async {
    ...
    update();
    bool success = false;
    ResponseModel responseModel = await couponServiceInterface.deleteCoupon(
      couponId,
    );
  ...
  Future<void> updateCoupon({
    ...
    ResponseModel responseModel = await couponServiceInterface.updateCoupon(
      data,
    );
    if (responseModel.isSuccess) {
      Get.back();
