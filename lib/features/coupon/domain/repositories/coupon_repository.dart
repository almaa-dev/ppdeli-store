...
import 'package:ppdelistore/features/coupon/domain/repositories/coupon_repository_interface.dart';
...
class CouponRepository implements CouponRepositoryInterface {
  ...
  Future<List<CouponBodyModel>?> getCouponList(int offset) async {
    ...
    Response response = await apiClient.getData(
      '${AppConstants.couponListUri}?limit=50&offset=$offset',
    );
    if (response.statusCode == 200) {
      coupons = [];
  ...
  Future<CouponBodyModel?> get(int? id) async {
    ...
    Response response = await apiClient.getData(
      '${AppConstants.couponDetailsUri}?coupon_id=$id',
    );
    if (response.statusCode == 200) {
  ...
  Future<ResponseModel> delete(int? id) async {
    ResponseModel responseModel;
    Response response = await apiClient.postData(AppConstants.couponDeleteUri, {
      "coupon_id": id,
    }, handleError: false);
    if (response.statusCode == 200) {
      responseModel = ResponseModel(true, response.body['message']);
