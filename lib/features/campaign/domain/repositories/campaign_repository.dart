...
import 'package:ppdelistore/features/campaign/domain/repositories/campaign_repository_interface.dart';
...
class CampaignRepository implements CampaignRepositoryInterface {
  ...
  Future<bool> joinCampaign(int? campaignID) async {
    Response response = await apiClient.putData(AppConstants.joinCampaignUri, {
  ...
  Future<bool> leaveCampaign(int? campaignID) async {
    Response response = await apiClient.putData(AppConstants.leaveCampaignUri, {
      'campaign_id': campaignID,
    });
    return (response.statusCode == 200);
  }
