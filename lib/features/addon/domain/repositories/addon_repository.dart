...
import 'package:ppdelistore/features/addon/domain/repositories/addon_repository_interface.dart';
...
class AddonRepository implements AddonRepositoryInterface<AddOns> {
  ...
  Future<bool> delete(int? id) async {
    Response response = await apiClient.deleteData(
      '${AppConstants.deleteAddonUri}?id=$id',
    );
    return (response.statusCode == 200);
  }
  ...
  Future<List<AddonCategoryModel>?> getAddonCategory({
    ...
    Response response = await apiClient.getData(
      '${AppConstants.addonCategoryList}?module_id=$moduleId',
    );
